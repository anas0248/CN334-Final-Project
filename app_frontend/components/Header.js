...
export default function Header() {
  ...
  return (
    <header className="absolute top-0 left-0 w-full z-20 px-8 py-4 flex justify-between items-center bg-transparent  text-yellow-400 font-instrument">
      ...
      {/* Navigation Menu */}
      <nav
        className={`absolute md:static text-center items-center top-16 left-0 w-full md:w-auto bg-transparent bg-black/60 md:bg-transparent md:flex items-center space-y-4 md:space-y-0 md:space-x-4 p-4 md:p-0 transition-all duration-300 ${isMenuOpen ? 'block' : 'hidden'
          }`}
      >
